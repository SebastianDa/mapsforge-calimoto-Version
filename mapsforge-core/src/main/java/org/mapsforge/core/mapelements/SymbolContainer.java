...
public class SymbolContainer extends MapElementContainer {
  	...
  	public void draw(Canvas canvas, Point topLeftPoint, Matrix matrix, Filter filter, double degreeRad) {
  	  	...
  	  	Point point = RotationUtil.getRotatedPoint(
  	  	  	  	this.xy.x - topLeftPoint.x,
  	  	  	  	this.xy.y - topLeftPoint.y,
  	  	  	  	degreeRad,
  	  	  	  	halfWidth,
  	  	  	  	halfHeight);
  	  	matrix.reset();
  	  	// We cast to int for pixel perfect positioning
  	  	matrix.translate((int) (point.x + boundary.left), (int) (point.y + boundary.top));
  	  	if (this.theta != 0) {
  	  	  	if (alignCenter) {
  	  	  	  	matrix.rotate(this.theta + (float) degreeRad, (float) point.x, (float) point.y);
  	  	  	} else {
  	  	  	  	matrix.rotate(this.theta + (float) degreeRad, (float) point.x, (float) point.y);
  	  	  	}
  	  	}
