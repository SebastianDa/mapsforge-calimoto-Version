...
public class WayTextContainer extends MapElementContainer {
  	...
  	@Override
  	public void draw(Canvas canvas, Point topLeftPoint, Matrix matrix, Filter filter, double degreeRad) {
  	  	Point adjustedStart = xy.offset(-topLeftPoint.x, -topLeftPoint.y);
  	  	Point adjustedEnd = end.offset(-topLeftPoint.x, -topLeftPoint.y);
  	  	
  	  	double halfWidth = 0.5*canvas.getWidth();
  	  	double halfHeight = 0.5*canvas.getHeight();
  	  	Point rotatedStart = RotationUtil.getRotatedPoint(
  	  	  	  	adjustedStart,
  	  	  	  	degreeRad,
  	  	  	  	halfWidth,
  	  	  	  	halfHeight);
  	  	Point rotatedEnd = RotationUtil.getRotatedPoint(
  	  	  	  	adjustedEnd,
  	  	  	  	degreeRad,
  	  	  	  	halfWidth,
  	  	  	  	halfHeight);
