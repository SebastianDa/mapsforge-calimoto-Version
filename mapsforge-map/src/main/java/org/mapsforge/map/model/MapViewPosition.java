...
public class MapViewPosition extends Observable implements Persistable {
  	...
  	private class MoveAnimator extends PausableThread {
  	
  	  	// debugging tip: for investigating what happens during the zoom animation
  	  	// just make the times longer for duration and frame length
  	  	private static final int DEFAULT_DURATION = 500;
  	  	private static final int FRAME_LENGTH_IN_MS = 20;
  	  	...
  	  	protected void doWork() throws InterruptedException {
  	  	  	if (System.currentTimeMillis() >= this.timeEnd) {
  	  	  	  	this.executeAnimation = false;
  	  	  	  	MapViewPosition.this.setCenter(this.pos);
  	  	  	} else {
  	  	  	  	long mapSize = MercatorProjection.getMapSize(
  	  	  	  	  	  	MapViewPosition.this.getZoomLevel(),
  	  	  	  	  	  	MapViewPosition.this.displayModel.getTileSize());
  	  	  	  	double targetPixelX = MercatorProjection.longitudeToPixelX(this.pos.longitude, mapSize);
  	  	  	  	double targetPixelY = MercatorProjection.latitudeToPixelY(this.pos.latitude, mapSize);
  	  	  	  	...
  	  	  	  	float timeElapsedRatio = (System.currentTimeMillis() - this.timeStart) / (1f * DEFAULT_DURATION);
  	  	  	  	double stepSizeX = (currentPixelX - targetPixelX) * timeElapsedRatio;
  	  	  	  	double stepSizeY = (currentPixelY - targetPixelY) * timeElapsedRatio;
  	  	  	  	MapViewPosition.this.moveCenter(stepSizeX, stepSizeY, false);
  	  	  	}
  	  	  	sleep(FRAME_LENGTH_IN_MS);
  	  	}
  	  	...
  	  	void startAnimation(LatLong pos) {
  	  	  	// TODO this is not properly synchronized
  	  	  	this.pos = pos;
  	  	  	this.executeAnimation = true;
  	  	  	this.timeStart = System.currentTimeMillis();
  	...
  	public synchronized void animateTo(LatLong pos) {
  	  	this.moveAnimator.startAnimation(pos);
  	}
  	
  	public synchronized void stopAnimateTo() {
  	  	this.moveAnimator.pause();
  	}
  	
  	public void destroy() {
  	  	this.zoomAnimator.interrupt();
  	  	this.moveAnimator.interrupt();
  	}
