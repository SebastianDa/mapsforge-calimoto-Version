...
public class LabelLayer extends Layer {
  	...
  	@Override
  	public void draw(BoundingBox boundingBox, byte zoomLevel, Canvas canvas, Point topLeftPoint, double degreeRad) {
  	  	...
  	  	draw(canvas, topLeftPoint, degreeRad);
  	}
  	
  	protected void draw(Canvas canvas, Point topLeftPoint, double degreeRad) {
  	  	for (MapElementContainer item : elementsToDraw) {
  	  	  	item.draw(canvas, topLeftPoint, this.matrix, this.displayModel.getFilter(), degreeRad);
  	  	}
  	}
}
