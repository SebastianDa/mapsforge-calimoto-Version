...
public final class MapPositionUtil {
  	public static BoundingBox getBoundingBox(MapPosition mapPosition, Dimension canvasDimension, int tileSize) {
  	  	...
  	  	int hypot = 1 + (int) Math.hypot(canvasDimension.width, canvasDimension.height) / 2;
  	
  	  	double pixelXMin = Math.max(0, pixelX - hypot);
  	  	double pixelYMin = Math.max(0, pixelY - hypot);
  	  	double pixelXMax = Math.min(mapSize, pixelX + hypot);
  	  	double pixelYMax = Math.min(mapSize, pixelY + hypot);
